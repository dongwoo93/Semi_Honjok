...
var num = 1;
	...
	$(document).ready(function() {
		...
		$("#likecancel").click(function() {
				...
				$('.comm').on('keyup','textarea', function(e){
					$(this).css('height', 'auto' );
			        $(this).height( this.scrollHeight );
				})
				$('.comm').find( 'textarea' ).keyup();
					})
</script>
          ...
          <br>
				...
				<c:when test="${id == 'admin'}">
					<tr align=right>
						<td colspan=4 height=20px align=right><input type="hidden"
							...
							<button type="button" id=tolist2>목록</button></td>
					</tr>
				</c:when>
				<c:otherwise>
					<tr align=right>
			...
			<form method=post action="comment.freeb" id=formid>
				<tr>
					...
					<td colspan=2 class="comm"><textarea id="comment" name=comment
