...
public class BoardTipDAO {
	...
	public List<BoardDTO> getSearchData(String keyword, int startNum, int endNum) throws Exception {
		Connection con = DBUtils.getConnection();
		String sql = "select * from (select admin_board.*, row_number() over(order by writedate desc) as num from admin_board where title like '%'||?||'%')\r\n" + 
				"where num between ? and ?";
		PreparedStatement pstat = con.prepareStatement(sql);
	...
	public String getKeywordNavi(int currentPage, String keyword) throws Exception {
		...
		if(needPrev) {
			//sb.append("<li class='page-item'><a class='page-link' href='select.tip?currentPage="+(startNavi-1)+"' class='navi'>"+"Previous"+"</a></li>");
			sb.append("<li class='page-item'><a class='page-link' href='searchtitle.tip?keyword="+keyword+"&currentPage="+(startNavi-1)+"' aria-label='Previous'> <span aria-hidden='true'>&laquo;</span>" + "<span class='sr-only'>"+"Previous"+"</span></a></li>");
			//sb.append("<a href='select.tip?currentPage="+(startNavi-1)+"' class='navi'>" + "< " + " </a>");
		}

		for(int i = startNavi; i <= endNavi; i++) {
			if(currentPage == i) {
				sb.append("<li class='page-item active'><a class='page-link' href='searchtitle.tip?keyword="+keyword+"&currentPage="+i+"' id="+i+">" + i + "</a></li>");
				//sb.append("<a href='select.tip?currentPage="+i+"' class='navi' id="+i+"> <b>" + i + "</b> </a>");
			}else {
				sb.append("<li class='page-item'><a class='page-link' href='searchtitle.tip?keyword="+keyword+"&currentPage="+i+"' id="+i+">" + i + "</a></li>");
				//sb.append("<a href='select.tip?currentPage="+i+"' class='navi' id="+i+">" + i + " </a>");
			}
		}
		...
		if(needNext) {
			//sb.append("<li class='page-item'><a class='page-link' href='select.tip?currentPage="+(endNavi+1)+"' class='navi'>" + "Next" + "</a></li>");
			sb.append("<li class='page-item'><a class='page-link' href='searchtitle.tip?keyword="+keyword+"&currentPage="+(endNavi+1)+"' aria-label='Next'> <span aria-hidden='true'>&raquo;</span>" + "<span class='sr-only'>" + "Next" + "</a></li>");
			//sb.append("<a href='select.tip?currentPage="+(endNavi+1)+"' class='navi'>" + ">" + " </a>");
		}
