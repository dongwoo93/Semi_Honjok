...
public class AdminFileDAO {
	...
	public List<AdminFilesDTO> getSubThum_sysFileName(int startNum, int endNum, String category, String subject) throws Exception {
		Connection con = DBUtils.getConnection();
		String sql = "select * from (select admin_files.*, row_number() over(order by article_no desc) as num from admin_files where category=? and subject=?)\r\n" + 
				"where num between ? and ?";
